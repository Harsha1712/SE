...
userApp.post("/createUser",upload.single("photo"),expressAsyncHandler(async(request,response)=>{
    ...
    let userOfDb=await userCollectionObject.findOne({accno:{$eq:newUserObj.accno}})
    if(userOfDb!==null)
    {
        response.send("Account number not available")
    }
    else{
...
userApp.post("/login",expressAsyncHandler(async(request,response)=>{
    ...
    let userOfDb= await userCollectionObject.findOne({accno:userCredObj.accno})
    if(userOfDb==null)
    {
        ...
        if(status==false)
        {
            ...
            let token = jwt.sign({accno:userOfDb.accno},process.env.SECRET_KEY,{expiresIn:10000000})
...
userApp.put("/mod",verifyToken,expressAsyncHandler(async(request,response)=>
{
    ...
    if(userofDb==null)
    {
        ...
        if(status==false)
        {
            ...
            await userCollectionObject.updateOne({accno:userCredObj.accno},{$set:{password:hashedPassword}})
            response.send({message:'success',userObj:userofDb})
        }
    }
}))
userApp.put("/mod1",verifyToken,expressAsyncHandler(async(request,response)=>
{
    let userCollectionObject=request.app.get("userCollectionObject")
    let userCredObj=request.body
    let userofDb = await userCollectionObject.findOne({accno:userCredObj.accno})
    if(userofDb==null)
    {
        ...
        await userCollectionObject.updateOne({accno:userCredObj.accno},{$set:{balance:(userofDb.balance-userCredObj.wd)}})
        response.send({message:'success',userObj:userofDb})
    }
}))
